...
# Get most important bigrams characters
def get_important_bigram_char(texts,n_feat=200):
    #output: a list of the N most important bigram characters from a list of sentences
...
for l in languages:
    ...
    #get 200 most important bigram chars
    important_bi_chars = get_important_bigram_char(texts, n_feat)

    #add to dict and set
    features[l] = important_bi_chars
    features_set.update(important_bi_chars) #the set of important bigram characters across all languages


#create vocabulary list using feature set of important bigram characters
vocab_dic = dict()
...
common_feature_matrix = [[0] * len(languages) for _ in range(len(languages))]

# Convert each list of language-features into a set
sets_of_features = [set(features[l]) for l in languages]

# Calculate the important bigram and fill in the matrix
for i in range(len(languages)):
    for j in range(0 , i+1):  # Avoid comparing the same list to itself and avoid duplicate comparisons
        common_features = sets_of_features[i].intersection(sets_of_features[j])
        common_feature_count = len(common_features)
        common_feature_matrix[i][j] = common_feature_count
        #common_feature_matrix[j][i] = common_feature_count  # Since it's symmetric

# The common_feature_matrix now contains the counts of common bigram between each pair of lists

fig, ax = plt.subplots(figsize=(len(languages), len(languages)))

# Create a heatmap of the common bigram chars matrix
cax = ax.matshow(common_feature_matrix, cmap="YlGnBu")
...
for i in range(len(languages)):
    for j in range(0 , i+1):
        count = common_feature_matrix[i][j]
        ax.text(j, i, str(count), ha="center", va="center", color="black")


# Add a title
plt.title("Common bigram chars Between Languages")
...
model = MultinomialNB()
train_x = train_feat.drop('labels',axis=1)
train_y = train_feat['labels']
...
conf_matrix = confusion_matrix(y_test, predict_test, labels = languages)
...
#test the prediction of some new texts
...
#true label for new tex is 'el' (modern greek)
# Create feature matrix for test_texts
